...
const GENERAL_CREDIT_PRICE = 37.5;

export default function AddMoreGeneralMembershipCreditModal({
  credits,
}: {
  ...
  const totalPrice = (quantity * GENERAL_CREDIT_PRICE).toFixed(2);
  ...
  const handleOk = () => {
    ...
    let newMachine = currentMachine + quantity;
    const finalMachine = newMachine > 4 ? 4 : newMachine;

    navigate("/general-membership-credit-payment", {
      state: {
        data: {
